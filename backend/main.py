...
async def text_to_speech(text: str) -> str:
    """Convert text to speech using OpenAI TTS and return base64 audio"""

    try:
        # Use OpenAI TTS API with 'alloy' voice (natural, friendly)
...
async def transcription(sid, data):
    ...
    if transcription_text and transcription_text.strip():
        ...
        if result["should_respond"]:
            print(
                f"[{result.get('current_stage', 'unknown')}] AI responding to {sid}: {result['response'][:50]}..."
            )

            # Generate TTS audio
            audio_base64 = await text_to_speech(result["response"])
            ...
            # Generate TTS audio even for non-response messages
            audio_base64 = await text_to_speech(result["response"])
...
async def code_update(sid, data):
    ...
    if (
        state.has_written_code
        and state.current_stage.value == "algorithm_design"
        and state.stage_progress.algorithm_traced
    ):
    ...
    print(
        f"[{state.current_stage.value}] Code updated for {sid}: {state.code_lines} lines"
    )
...
async def end_interview(sid, data):
    ...
    if sid not in active_sessions:
        await sio.emit(
            "interview_ended",
            {"content": "Interview session not found.", "feedback": None},
            room=sid,
        )
        return
    ...
    print(
        f"Generated feedback for {sid}: Grade {feedback.overall_grade} ({feedback.overall_score:.2f})"
    )
